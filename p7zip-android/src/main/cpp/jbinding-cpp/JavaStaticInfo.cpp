...
namespace jni {
    ...
    void JMethod::initMethodID(JNIEnv *env, jclass jclazz) {
        ...
        if (env->ExceptionOccurred()) {
            ...
            env->ExceptionClear();
            if (jni::NoSuchMethodError::_isInstance(env, exception.get())) {
                return;
            }
            if (jni::OutOfMemoryError::_isInstance(env, exception.get())) {
                FATAL("Out of memory during method lookup: '%s', '%s'", _name,
                      _signature); // TODO Change fatal => exception (+test)
            }
            if (jni::ExceptionInInitializerError::_isInstance(env, exception.get())) {
                FATAL("Exception in initializer during method lookup: '%s', '%s'", _name,
                      _signature); // TODO Change fatal => exception (+test)
            }
