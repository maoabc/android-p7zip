...
public class VolumedArchiveInStream implements IInStream {
    ...
    private static final String ZIP_FIRST_VOLUME_POSTFIX = ".zip.001";
     ...
     * will be thrown.
     *
     * @param archiveOpenVolumeCallback call back implementation used to access different volumes of archive.
     ...
     *
     * @param firstVolumeFilename       the file name of the first volume.
     * @param archiveOpenVolumeCallback call back implementation used to access different volumes of archive. The file name should ends with
    ...
    public VolumedArchiveInStream(String firstVolumeFilename, IArchiveOpenVolumeCallback archiveOpenVolumeCallback)
        ...
        volumePositions.add(0L);

        if (!firstVolumeFilename.endsWith(SEVEN_ZIP_FIRST_VOLUME_POSTFIX)
                && !firstVolumeFilename.endsWith(ZIP_FIRST_VOLUME_POSTFIX)
        ) {
            throw new SevenZipException("The first 7z or zip volume filename '" + firstVolumeFilename
                    + "' don't ends with the postfix: '" + SEVEN_ZIP_FIRST_VOLUME_POSTFIX + " or " + ZIP_FIRST_VOLUME_POSTFIX + "'. Can't proceed");
    ...
    private void openVolume(int index, boolean seekToBegin) throws SevenZipException {
        ...
        String volumeFilename = cuttedVolumeFilename + MessageFormat.format("{0,number,000}", index);
        ...
        if (volumePositions.size() == index) {
            ...
            volumePositions.add(volumePositions.get(index - 1) + currentVolumeLength);

            if (seekToBegin) {
                currentInStream.seek(0, SEEK_SET);
            }
        } else {
            currentVolumeLength = volumePositions.get(index) - volumePositions.get(index - 1);
        }

        if (seekToBegin) {
            currentVolumeOffset = 0;
            absoluteOffset = volumePositions.get(index - 1);
        }
    ...
    private void openVolumeToAbsoluteOffset() throws SevenZipException {
        ...
        while (volumePositions.get(index) > absoluteOffset) {
            index--;
        }
        ...
        do {
            ...
            openVolume(index, false);
                ...
                && volumePositions.get(index) <= absoluteOffset);
    ...
    public synchronized long seek(long offset, int seekOrigin) throws SevenZipException {
        ...
        currentVolumeOffset = absoluteOffset - volumePositions.get(currentIndex - 1);
    ...
    public int read(ByteBuffer src, int len) throws SevenZipException {
        ...
        int read = currentInStream.read(src, len);
    ...
    public void close() throws IOException {
        if (archiveOpenVolumeCallback instanceof Closeable) {
            ((Closeable) archiveOpenVolumeCallback).close();
        }
    }
}
